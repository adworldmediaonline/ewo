...
export default async function CategoryItems() {
  try {
    ...
    if (!data || typeof data !== 'object') {
      ...
      return <div className={styles.ewoError}>Unable to load categories</div>;
    }

    // Check if result exists and is an array
    if (!Array.isArray(data?.result)) {
      console.error(
        'API response does not contain a valid result array:',
        data
      );
      return <div className={styles.ewoError}>No categories available</div>;
    }
          ...
          {filteredCategories.map((category, index) => (
            ...
            >
              <div className={styles.ewoImageWrapper}>
                <Image
                  src={category.img || '/placeholder-image.jpg'}
                  alt={category.parent || 'Category'}
                  width={240}
                  height={240}
              ...
              <div className={styles.ewoContent}>
                <h3 className={styles.ewoCategoryTitle}>
                  {titleCaseFirstLetterOfEveryWord(
